...
Loads PDF materials and splits text into manageable chunks using recursive character text splitting.
...
API keys required:
LANGCHAIN_TRACING_V2 for LangChain tracing.
GOOGLE_API_KEY for Google Gemini model integration.
Setup Instructions:
Install dependencies via the requirements.txt file.
Ensure you have a valid LANGCHAIN_TRACING_V2 and GOOGLE_API_KEY to use LangChain and Google APIs.
Set up MongoDB for storing and retrieving student data.
Run the app using FastAPI, accessible at http://localhost:8000/.
Additional Notes:
